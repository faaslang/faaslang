...
const _httpResponse = {
  ...
  'object.http': (v, headers) => {
    headers = Object.keys(headers || {}).reduce((casedHeaders, headerName) => {
      casedHeaders[headerName.toLowerCase()] = headers[headerName];
      return casedHeaders;
    }, {});
    v.headers = Object.keys(v.headers || {}).reduce((mergedHeaders, headerName) => {
      if (!mergedHeaders[headerName.toLowerCase()]) {
        mergedHeaders[headerName.toLowerCase()] = v.headers[headerName];
      }
